 ...
 * timer/counter.
 */
static inline void dispatchReceivedDataEdge(volatile RxPort *rxPort, uint8_t isRisingEdge) {
    uint16_t hardwareCounter = TIMER_TX_RX_COUNTER1;

    // if there is no ongoing reception thus this is the first signal of a package
    if (rxPort->isReceiving == 0) {
        ...
        if ((DEFAULT_TX_RX_COUNTER_LEFT_OF_CENTER_VALUE <= synchronizedCaptureCounter) &&
            (synchronizedCaptureCounter <= DEFAULT_TX_RX_COUNTER_LEFT_OF_CENTER_VALUE)) {
            reApproximateTimerCounterMaximumValueOnCenter(&synchronizedCaptureCounter);
            storeDataBit(rxPort, isRisingEdge);
        }
 ...
 */
static inline void storeDataBit(volatile RxPort *rxPort, uint8_t isRisingEdge) {

    if (isBufferFull(&(rxPort->buffer.pointer)) != 1) {

        // save bit to buffer
        if (isRisingEdge == 0) {
            rxPort->buffer.bytes[rxPort->buffer.pointer.byteNumber]
                    unsetBit
                            rxPort->buffer.pointer.bitMask;
        } else {
            rxPort->buffer.bytes[rxPort->buffer.pointer.byteNumber]
                    setBit
                    rxPort->buffer.pointer.bitMask;
