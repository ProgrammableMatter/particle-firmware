 ...
 */
#define MIN_RX_NEIGHBOUR_SIGNALS_SENSE ((uint8_t)(RX_DISCOVERY_PULSE_COUNTER_MAX / 2.0)  + 1)
/**
 * Earliest particle loop when local node discovery may be finished.
 */
#define MIN_NEIGHBOURS_DISCOVERY_LOOPS ((uint8_t)(5 * RX_DISCOVERY_PULSE_COUNTER_MAX))
/**
 * Latest particle loop when local node discovery is to be aborted.
 */
#define MAX_NEIGHBOURS_DISCOVERY_LOOPS ((uint8_t)(2 * MIN_NEIGHBOURS_DISCOVERY_LOOPS))
/**
 * Latest particle loop when pulsing to neighbours is to be deactivated.
 */
#define MAX_NEIGHBOUR_PULSING_LOOPS ((uint8_t)(3 * MIN_NEIGHBOURS_DISCOVERY_LOOPS))

/**
 * Neighbour discovery counter 1 compare A value.
 */
#define DEFAULT_NEIGHBOUR_SENSING_COMPARE_VALUE ((uint16_t)0x10)
